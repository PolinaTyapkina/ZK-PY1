...
def to_csv_file(headers=headers_list, rows=data, filename=OUTPUT_FILE, delimiter=',', new_line='\n'):
    with open(filename, 'w', encoding='utf8') as filename:
        filename.writelines(head + delimiter for head in headers[:-1])
        filename.writelines(head for head in headers[-1])
        for _ in data:
            filename.writelines(new_line)
            filename.writelines(number + delimiter for number in _[:-1])
            filename.writelines(number for number in _[-1])
        filename.writelines(new_line)
